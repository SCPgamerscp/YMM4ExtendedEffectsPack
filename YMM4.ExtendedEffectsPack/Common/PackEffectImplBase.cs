...
internal abstract class PackEffectImplBase<T> : D2D1CustomShaderEffectImplBase<T> where T : PackEffectImplBase<T>
{
    protected PackUniforms _cb;

    protected PackEffectImplBase(string shaderName) : base(ShaderResourceUri.Get(shaderName)) {}
    ...
    {
        float w = inputRect.Right - inputRect.Left;
        float h = inputRect.Bottom - inputRect.Top;
        if (w > 0 && h > 0)
        {
            _cb.BoundsLeft = inputRect.Left;
            _cb.BoundsTop = inputRect.Top;
            _cb.BoundsWidth = w;
            _cb.BoundsHeight = h;
        }
        else
